...
class Myepisode_m extends CI_Model {
	...
	public function listepisode(){
		$this->db->where('episode.iduser', $this->session->userdata('c_id'));
		$this->db->order_by("idepisode", "desc");
	...
	public function get_iduser_episode($id){
		$this->db->where('idepisode', $id);
		$sdf = $this->db->get('episode')->row('iduser');
		return $sdf;
	}
}
